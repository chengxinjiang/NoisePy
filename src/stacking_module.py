...
# This module contains all of the tested stacking routine from Yang et al, 2020. 
# Core methods are: pws (phase weighted stacking), acfs (auto-covariance filter stacking),
#  robusts (robust stacking), nroots (nroot stacking), selectives (selective stacking)
...
def pws(cc_array,sampling_rate,power=2,pws_timegate=5.):
    ...
    analytic = hilbert(cc_array,axis=1, N=scipy.fftpack.next_fast_len(M))[:,:M]
...
def acfs(cc_array,n):
    ...
    n: a positive number to adjust the filter harshness
    RETURNS:
    ----------------------
    ...
    if cc_array.ndim == 1:
        ...
        return cc_array
    N,M = cc_array.shape
    Nfft = scipy.fftpack.next_fast_len(M)
...
def selectives(cc_array,epsilon,cc_th):
    ''' 
